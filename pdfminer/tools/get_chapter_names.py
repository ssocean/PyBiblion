...
def dumpoutline(
    ...
    try:
        ...
        for (level, title, dest, a, se) in outlines:
            ...
            if level==1:
                outfp.write(s)
                first_headings.append(s)
                words = s.split(' ')
                for word in words:
                    if s in key_word_freq:
                        key_word_freq[word.lower()] += 1
                    else:
                        key_word_freq[word.lower()] = 1
        num_headings = len(first_headings)
        normalized_location = list(np.linspace(0, 1, num_headings))

        for p,headding in enumerate(first_headings):
            for word in headding.split(' '):
                if word in key_word_hist:
                    key_word_hist[word.lower()].append(normalized_location[p])
                else:
                    key_word_hist[word.lower()] = [normalized_location[p]]
            # if dest is not None:
            #     outfp.write("<dest>")
            #     dumpxml(outfp, dest)
            #     outfp.write("</dest>\n")
    ...
    fp.close()
    # print(key_word_freq)
    return key_word_freq,key_word_hist
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    for fname in files:
        # print(fname)
        try:
            freq_dict,keyword_hist = dumpoutline(
                outfp,
                os.path.join(args.files,fname),
                objids,
                pagenos,
                password=password,
                dumpall=args.all,
        ...
        word_dis_hist_dict = merge_dicts(word_dis_hist_dict,keyword_hist)
    ...
    total_freq_dict = dict(sorted(total_freq_dict.items(), key=lambda x: x[1]))
    print(total_freq_dict)
    save_dict_to_json(total_freq_dict, 'total_freq_dict.json')
    print(word_dis_hist_dict)
    save_dict_to_json(word_dis_hist_dict, 'word_dis_hist_dict.json')
    outfp.close()
